...
#   implemented_properties = ['energy', 'forces', 'dipole']
    implemented_properties = ['energy', 'forces', 'dipole', 'molar_volume', 'molecular_entropy', 'mulliken_charge', 'nbo_charge', 'mo_energy', 'mo_coeff' ]
...
# ishi-begin
        if self.parameters.has_key('force') and self.parameters['force'] is None:
...
# ishi-end
    ...
    def read_results(self):
        ...
        filename = self.label + '.log'
# ishi-begin
        quantities = ['energy', 'dipole']

        if self.parameters.has_key('force'):
            quantities.append('forces')

        if self.parameters.has_key('volume'):
            quantities.append('molar_volume')

        if self.parameters.has_key('freq'):
            quantities.append('molecular_entropy')

        if self.parameters.has_key('population'):
            if 'nbo' in self.parameters['population'].lower():
                quantities.append('nbo_charge')
            quantities.append('mulliken_charge')
            quantities.append('mo_energy')
            quantities.append('mo_coeff')

        if self.parameters.has_key('pop'):
            if 'nbo' in self.parameters['pop'].lower():
                quantities.append('nbo_charge')
            quantities.append('mulliken_charge')
            quantities.append('mo_energy')
            quantities.append('mo_coeff')
